...
const login = async (req, res, next) => {
  try {
    if (
      ...
      return next(createError(403, "Kindly fill the required fields", false));

    const staff = await Staff.findOne({ email: req.body.email });

    if (!staff) return next(createError(404, "staff Email not found!", false));
    ...
    if (!isPasswordCorrect) {
      return next(
        createError(400, "Incorrect Password, Kindly Try Again!", false)
      );
    }
...
const forgetPWD = async (req, res, next) => {
  try {
    ...
    if (!existingUser)
      next(createError(400, "Email Address provided does not Exist!"));
